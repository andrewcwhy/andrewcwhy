# Andrew Christian Young

---

## 👋 About Me

The Three I Am’s

- I'm Andrew!
- I'm a IT major at the University of Central Florida.
- I'm building [Janustack](https://github.com/janustack) — an organization focused on creating developer tools in TypeScript and Zig

---

## 💻 Technical Interests

- Cybersecurity
- Linux system administration and server management
- Systems programming
- Web development
---

## 🔍 Currently Exploring

- Linux
- JavaScript
- Zig

---

## 🛠️ Favorite Tools

If it’s fast, safe, and clean, I’m into it. These are the tools that I get hard for:

- [Bun](https://bun.sh/) - All-in-one JavaScript runtime and toolkit written Zig.
- [Fish](https://fishshell.com/) - Command-line shell written in Rust.
- [Ghostty](https://ghostty.org/) - Terminal emulator that uses platform-native UI and GPU acceleration written in Zig.
- [Helix](https://helix-editor.com/) - A modal code editor for the terminal, built in Rust.
- [Moon](https://moonrepo.dev/moon) - A task runner and monorepo management tool for the web ecosystem written in Rust.
- [Proto](https://moonrepo.dev/proto) - A unified toolchain and version manager for all my goto languages and tools written in Rust.
- [SolidJS](https://www.solidjs.com/) - Simple and performant reactivity for building user interfaces.
- [Zed](https://www.zed.dev/) - A high-performance, GPU-accelerated code editor written in Rust.
- [Zig](https://ziglang.org/) - Yeah, I use Zig's build system and cross-compilimation toolchain for C projects.

---

> Visit my site: [andrewcwhy.me](https://andrewcwhy.me)
